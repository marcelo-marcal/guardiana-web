...
export default function ConfiguracoesPage() {
    ...
    useEffect(() => {
        const fetchSetting = async () => {
            try {
                const res = await fetch(`${API_URL}/auth/settings/show_poems_section`);
    ...
    const handleToggle = async () => {
        ...
        try {
            ...
            if (!token) throw new Error("Sessão não encontrada. Por favor, faça login novamente.");
            ...
            if (!res.ok) {
                throw new Error(data.error || data.message || "Erro ao atualizar configuração.");
            }
    ...
    return (
        <div className="p-8 max-w-4xl mx-auto">
            <header className="mb-10">
                <h1 className="text-3xl font-bold text-[#18384A] dark:text-white">Configurações do Sistema</h1>
            ...
            <div className="bg-white dark:bg-[#0F1720] rounded-3xl border border-gray-200 dark:border-white/10 overflow-hidden shadow-sm">
                <div className="p-8 border-b border-gray-100 dark:border-white/5">
                    <div className="flex items-center justify-between">
                        <div className="max-w-xl">
                            <h2 className="text-lg font-bold text-[#18384A] dark:text-white">Seção de Poemas (Página Inicial)</h2>
                            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                                Ative ou desative a exibição da seção "Poesias em Destaque" na página inicial do site.
                            </p>
                        </div>
                        ...
                        <button
                            ...
                            className={`${
                                isVisible ? "bg-[#C95F52]" : "bg-gray-200 dark:bg-gray-700"
                ...
                <div className="p-6 bg-gray-50 dark:bg-white/5">
                    <p className="text-xs text-gray-400 italic">
                        * Alterações nesta seção impactam imediatamente a visibilidade para todos os visitantes do site.
                    </p>
                </div>
            </div>
        </div>
