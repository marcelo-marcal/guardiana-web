...
export default function PoemasDestaque() {
    ...
    useEffect(() => {
        const fetchHighlights = async () => {
            try {
                // Primeiro verifica se a seção deve estar visível
                const settingsRes = await fetch(`${API_URL}/auth/settings/show_poems_section`);
    ...
    if (!loading && (!isVisible || poems.length === 0)) return null;

    return (
        <section className="relative w-full py-24 bg-white dark:bg-[#020617] overflow-hidden transition-colors">
            {/* Decoração sutil de fundo */}
            <div className="absolute top-0 right-0 w-64 h-64 opacity-5 pointer-events-none">
                <Image src="/logo-grande.png" alt="" fill className="object-contain grayscale" />
            </div>
                        ...
                        {poems.map((poem) => (
                            ...
                            >
                                <div>
                                    <div className="flex items-center gap-3 mb-6">
                                        <div className="w-10 h-10 rounded-full bg-[#18384A] flex items-center justify-center text-white font-bold text-sm overflow-hidden border-2 border-[#D4AF37]">
                                            {poem.user.avatarUrl ? (
                                                <Image src={poem.user.avatarUrl} alt={poem.user.name} width={40} height={40} />
                                            ) : (
                                                poem.user.name.charAt(0).toUpperCase()
                                            )}
                                        </div>
                                        <div className="flex flex-col">
                                            <span className="text-xs font-bold text-[#C95F52] uppercase tracking-wider">Autor(a)</span>
                                ...
                                <div className="mt-6 pt-4 border-t border-gray-200 dark:border-white/5 flex justify-between items-center text-[10px] text-gray-400 uppercase font-medium">
                                    ...
                                    <span>{new Date(poem.createdAt).toLocaleDateString('pt-BR')}</span>
                                </div>
                            </div>
                        ))}
                    </div>
